...
exports.send_control = function (ip_address, opts, callback) {    
	var target="http://" + ip_address;
	var request="POST";
                ...
                if (responseData.ret) {
                    ...
                    switch (responseData.ret) {
                        ...
                        case 'PARAM NG':    delete responseData.ret;
                                            callback('Wrong Parameters in request: ' + opts, ret, responseData);
                                            return;
                        case 'ADV NG':      delete responseData.ret;
                                            callback('Wrong ADV: ' + opts, ret, responseData);
                                            return;
                        default:            callback('Unknown response: ' + opts, ret, responseData);
                                            return;
                    }
                }
                else {
                    callback('Unknown response: ' + opts, ret, responseData);
                }           
                //var response = JSON.parse(xmlhttp.responseText);
			}else{
				callback("Send control request failed !!", xmlhttp.status, "-");
			}
		}else{
			//alert(xmlhttp.readyState);
		}
...
exports.minimize_opt = function (opt) {
	var min_opt = {};
